...
public class Player extends AbstractEntity implements PlayerIf {
	...
	if (direction == 1) {
		...
		if (block.getX() == (int) this.x && block.getY() == getY()) {
		    return;
		}
	    }
	} else if (direction == 2) {
	    for (AbstractBlock block : gameLogic.getAllBlocks()) {
		if (block.getX() == getX() && block.getY() == (int) this.y) {
		    return;
		}
	    }
	} else if (direction == 3) {
	    for (AbstractBlock block : gameLogic.getAllBlocks()) {
		if (block.getX() == (int) this.x + 1 && block.getY() == getY()) {
		    return;
		}
	    }
	} else if (direction == 4) {
	    for (AbstractBlock block : gameLogic.getAllBlocks()) {
		if (block.getX() == getX() && block.getY() == (int) this.y + 1) {
		    return;
		}
	    }
	}
